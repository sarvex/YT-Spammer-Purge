...
def validate_post_id(post_url):
    ...
    if len(isolatedPostID) not in {26, 36}:
        return False, None, None, None, None
    if isolatedPostID[:2] == "Ug":
        validatedPostUrl = f"https://www.youtube.com/post/{isolatedPostID}"
...
def validate_channel_id(inputted_channel):
    ...
    elif all(x not in inputted_channel for x in notChannelList) and (
    ...
    if len(isolatedChannelID) == 24 and isolatedChannelID[:2] == "UC":
        ...
        if response['items']:
            return True, isolatedChannelID, response['items'][0]['snippet']['title']
